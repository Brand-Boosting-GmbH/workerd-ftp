...
type FTPClientConstructor = new (host: string, opts?: ConnectionOptions) => _FTPClient
...
beforeAll(async () => {
  ...
  for (const file of files) {
    await (file.isFile ? basicFtp.remove(file.name) : basicFtp.removeDir(file.name))
  }
})
...
describe('FTPClient', () => {
  ...
  test('listAfterUpload', async () => {
    ...
    expect(list.filter((i) => !['', '.', '..'].includes(i))).toEqual(['test.txt'])
  })


  test('textDownload', async () => {
  ...
  test('TODO: unsecure textUpload', async () => {
    await run(async () => {
      const ftp = new FTPClient('$SERVER$', {
        port: 21,
        user: '$USER$',
